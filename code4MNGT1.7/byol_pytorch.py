...
class NetWrapper(nn.Module):
    ...
    def get_representation(self, x):
        pred, classresult = self.net(x)
        return pred, classresult
    ...
    def forward(self, x, return_projection=True):
        representation, classresult = self.get_representation(x)
        ...
        return projection, representation, classresult
        # return 1, 1, classresult
#
...
class BYOL(nn.Module):
    ...
    def forward(
        ...
        image_one, image_two = self.augment1(img1), self.augment1(img2)

        online_proj_one, _, classresult_one = self.online_encoder(image_one)
        online_proj_two, _, classresult_two = self.online_encoder(image_two)
        ...
        with torch.no_grad():
            ...
            target_proj_one, _, _ = target_encoder(image_one)
            target_proj_two, _, _ = target_encoder(image_two)
